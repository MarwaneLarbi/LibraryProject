<div>
    <div class="card mt-5 mt-xxl-8">
        <!--begin::Card head-->
        <div class="card-header card-header-stretch">
            <!--begin::Title-->
            <div class="card-title d-flex align-items-center">
                <!--begin::Svg Icon | path: icons/duotune/general/gen014.svg-->
                <span class="svg-icon svg-icon-1 svg-icon-primary me-3 lh-0">
												<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
													<path opacity="0.3" d="M21 22H3C2.4 22 2 21.6 2 21V5C2 4.4 2.4 4 3 4H21C21.6 4 22 4.4 22 5V21C22 21.6 21.6 22 21 22Z" fill="black"></path>
													<path d="M6 6C5.4 6 5 5.6 5 5V3C5 2.4 5.4 2 6 2C6.6 2 7 2.4 7 3V5C7 5.6 6.6 6 6 6ZM11 5V3C11 2.4 10.6 2 10 2C9.4 2 9 2.4 9 3V5C9 5.6 9.4 6 10 6C10.6 6 11 5.6 11 5ZM15 5V3C15 2.4 14.6 2 14 2C13.4 2 13 2.4 13 3V5C13 5.6 13.4 6 14 6C14.6 6 15 5.6 15 5ZM19 5V3C19 2.4 18.6 2 18 2C17.4 2 17 2.4 17 3V5C17 5.6 17.4 6 18 6C18.6 6 19 5.6 19 5Z" fill="black"></path>
													<path d="M8.8 13.1C9.2 13.1 9.5 13 9.7 12.8C9.9 12.6 10.1 12.3 10.1 11.9C10.1 11.6 10 11.3 9.8 11.1C9.6 10.9 9.3 10.8 9 10.8C8.8 10.8 8.59999 10.8 8.39999 10.9C8.19999 11 8.1 11.1 8 11.2C7.9 11.3 7.8 11.4 7.7 11.6C7.6 11.8 7.5 11.9 7.5 12.1C7.5 12.2 7.4 12.2 7.3 12.3C7.2 12.4 7.09999 12.4 6.89999 12.4C6.69999 12.4 6.6 12.3 6.5 12.2C6.4 12.1 6.3 11.9 6.3 11.7C6.3 11.5 6.4 11.3 6.5 11.1C6.6 10.9 6.8 10.7 7 10.5C7.2 10.3 7.49999 10.1 7.89999 10C8.29999 9.90003 8.60001 9.80003 9.10001 9.80003C9.50001 9.80003 9.80001 9.90003 10.1 10C10.4 10.1 10.7 10.3 10.9 10.4C11.1 10.5 11.3 10.8 11.4 11.1C11.5 11.4 11.6 11.6 11.6 11.9C11.6 12.3 11.5 12.6 11.3 12.9C11.1 13.2 10.9 13.5 10.6 13.7C10.9 13.9 11.2 14.1 11.4 14.3C11.6 14.5 11.8 14.7 11.9 15C12 15.3 12.1 15.5 12.1 15.8C12.1 16.2 12 16.5 11.9 16.8C11.8 17.1 11.5 17.4 11.3 17.7C11.1 18 10.7 18.2 10.3 18.3C9.9 18.4 9.5 18.5 9 18.5C8.5 18.5 8.1 18.4 7.7 18.2C7.3 18 7 17.8 6.8 17.6C6.6 17.4 6.4 17.1 6.3 16.8C6.2 16.5 6.10001 16.3 6.10001 16.1C6.10001 15.9 6.2 15.7 6.3 15.6C6.4 15.5 6.6 15.4 6.8 15.4C6.9 15.4 7.00001 15.4 7.10001 15.5C7.20001 15.6 7.3 15.6 7.3 15.7C7.5 16.2 7.7 16.6 8 16.9C8.3 17.2 8.6 17.3 9 17.3C9.2 17.3 9.5 17.2 9.7 17.1C9.9 17 10.1 16.8 10.3 16.6C10.5 16.4 10.5 16.1 10.5 15.8C10.5 15.3 10.4 15 10.1 14.7C9.80001 14.4 9.50001 14.3 9.10001 14.3C9.00001 14.3 8.9 14.3 8.7 14.3C8.5 14.3 8.39999 14.3 8.39999 14.3C8.19999 14.3 7.99999 14.2 7.89999 14.1C7.79999 14 7.7 13.8 7.7 13.7C7.7 13.5 7.79999 13.4 7.89999 13.2C7.99999 13 8.2 13 8.5 13H8.8V13.1ZM15.3 17.5V12.2C14.3 13 13.6 13.3 13.3 13.3C13.1 13.3 13 13.2 12.9 13.1C12.8 13 12.7 12.8 12.7 12.6C12.7 12.4 12.8 12.3 12.9 12.2C13 12.1 13.2 12 13.6 11.8C14.1 11.6 14.5 11.3 14.7 11.1C14.9 10.9 15.2 10.6 15.5 10.3C15.8 10 15.9 9.80003 15.9 9.70003C15.9 9.60003 16.1 9.60004 16.3 9.60004C16.5 9.60004 16.7 9.70003 16.8 9.80003C16.9 9.90003 17 10.2 17 10.5V17.2C17 18 16.7 18.4 16.2 18.4C16 18.4 15.8 18.3 15.6 18.2C15.4 18.1 15.3 17.8 15.3 17.5Z" fill="black"></path>
												</svg>
											</span>
                <!--end::Svg Icon-->
                <h3 class="fw-bolder m-0 text-gray-800">{{ \Carbon\Carbon::now() }}</h3>
            </div>
            <!--end::Title-->
            <!--begin::Toolbar-->
            <div class="card-toolbar m-0">
                <!--begin::Tab nav-->
                <ul class="nav nav-tabs nav-line-tabs nav-stretch fs-6 border-0 fw-bolder" role="tablist">
                    <li class="nav-item" role="presentation">
                        <a id="kt_activity_today_tab" class="nav-link justify-content-center text-active-gray-800" data-bs-toggle="tab" role="tab" href="#kt_activity_today" aria-selected="false">Today</a>
                    </li>
                    <li class="nav-item" role="presentation">
                        <a id="kt_activity_month_tab" class="nav-link justify-content-center text-active-gray-800" data-bs-toggle="tab" role="tab" href="#kt_activity_month" aria-selected="false">Month</a>
                    </li>
                    <li class="nav-item" role="presentation">
                        <a id="kt_activity_year_tab" class="nav-link justify-content-center text-active-gray-800 text-hover-gray-800 active" data-bs-toggle="tab" role="tab" href="#kt_activity_year" aria-selected="true">2021</a>
                    </li>
                </ul>
                <!--end::Tab nav-->
            </div>
            <!--end::Toolbar-->
        </div>
        <!--end::Card head-->
        <!--begin::Card body-->
        <div class="card-body">
            <!--begin::Tab Content-->
            <div class="tab-content">
                <!--begin::Tab panel-->
                <div id="kt_activity_today" class="card-body p-0 tab-pane fade" role="tabpanel" aria-labelledby="kt_activity_today_tab">
                    <div class="d-flex flex-row p-2">

                        <input type="text" data-kt-docs-table-filter="search" class="form-control form-control-solid w-250px ps-15 " placeholder="Search Emprunts"/>
                    </div>
                    <!--begin::Timeline-->
                    <div class="timeline" >

                        <table id="kt_datatable_days" class="table align-middle table-row-dashed fs-6 gy-5">
                            <thead>
                            <tr class="fw-bold fs-6 text-muted">
                                <th>action</th>
                            </tr>
                            </thead>
                            <tbody class="text-gray-600 fw-bold">

                                @foreach($todays as $emprunt)
                                    <tr> @if($emprunt->status=='active')
                                        <td>    <div class="timeline-item content">

                                            <div class="timeline-icon symbol symbol-circle symbol-40px me-4">
                                                <div class="symbol-label bg-light">
                                                    <!--begin::Svg Icon | path: icons/duotune/communication/com003.svg-->

                                                    <span class="svg-icon svg-icon-1 svg-icon-primary ">
												<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
										 <path opacity="0.5" d="M12.8956 13.4982L10.7949 11.2651C10.2697 10.7068 9.38251 10.7068 8.85731 11.2651C8.37559 11.7772 8.37559 12.5757 8.85731 13.0878L12.7499 17.2257C13.1448 17.6455 13.8118 17.6455 14.2066 17.2257L21.1427 9.85252C21.6244 9.34044 21.6244 8.54191 21.1427 8.02984C20.6175 7.47154 19.7303 7.47154 19.2051 8.02984L14.061 13.4982C13.7451 13.834 13.2115 13.834 12.8956 13.4982Z" fill="black"/>
                                        <path d="M7.89557 13.4982L5.79487 11.2651C5.26967 10.7068 4.38251 10.7068 3.85731 11.2651C3.37559 11.7772 3.37559 12.5757 3.85731 13.0878L7.74989 17.2257C8.14476 17.6455 8.81176 17.6455 9.20663 17.2257L16.1427 9.85252C16.6244 9.34044 16.6244 8.54191 16.1427 8.02984C15.6175 7.47154 14.7303 7.47154 14.2051 8.02984L9.06096 13.4982C8.74506 13.834 8.21146 13.834 7.89557 13.4982Z" fill="black"/>
                                       </svg>
											</span>
                                                    <!--end::Svg Icon-->
                                                </div>
                                            </div>
                                            <!--end::Timeline icon-->
                                            <!--begin::Timeline content-->
                                            <div class="timeline-content mb-10 mt-n1">
                                                <!--begin::Timeline heading-->
                                                <div class="pe-3 mb-5">
                                                    <!--begin::Title-->
                                                    <div class="fs-5 fw-bold mb-2">
                                                        <span class="text-success">
                                                          @if(\App\Models\abonne::where( 'id',$emprunt->abonne_id)->count()!=null)
                                                                {{
                                                                    \App\Models\abonne::find($emprunt->abonne_id)->nom}}
                                                                {{\App\Models\abonne::find($emprunt->abonne_id)->prenom}}
                                                            @endif
                                                        </span>


                                                        a emprenté le  livre
                                                        <span class="text-uppercase"> "
                                                            @if(\App\Models\livre::where('id',$emprunt->livre_id)->count()!=null)
                                                                {{ \App\Models\livre::find($emprunt->livre_id)->titre }}
                                                            @endif
                                                        </span>
                                                    </div>
                                                    <!--end::Title-->
                                                    <!--begin::Description-->
                                                    <div class="d-flex align-items-center mt-1 fs-6">
                                                        <!--begin::Info-->
                                                        <div class="text-muted me-2 fs-7">Emprunts at {{$emprunt->date}} by <span class="text-success">  {{$emprunt->abonne_id }}  </span> </div>
                                                        <!--end::Info-->
                                                        <!--begin::User-->

                                                        <!--end::User-->
                                                    </div>
                                                    <!--end::Description-->
                                                </div>
                                                <!--end::Timeline heading-->
                                                <!--begin::Timeline details-->
                                                <!--end::Timeline details-->
                                            </div>
                                            <!--end::Timeline content-->
                                        </div></td>
                                    @else
                                        <td>    <div class="timeline-item content">
                                            <!--begin::Timeline line-->
                                            <!--end::Timeline line-->
                                            <!--begin::Timeline icon-->
                                            <div class="timeline-icon symbol symbol-circle symbol-40px">
                                                <div class="symbol-label bg-light">
                                                    <!--begin::Svg Icon | path: icons/duotune/communication/com009.svg-->
                                                    <span class="svg-icon svg-icon-1 svg-icon-danger ">
	<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
<path opacity="0.5" d="M14.2657 11.4343L18.45 7.25C18.8642 6.83579 18.8642 6.16421 18.45 5.75C18.0358 5.33579 17.3642 5.33579 16.95 5.75L11.4071 11.2929C11.0166 11.6834 11.0166 12.3166 11.4071 12.7071L16.95 18.25C17.3642 18.6642 18.0358 18.6642 18.45 18.25C18.8642 17.8358 18.8642 17.1642 18.45 16.75L14.2657 12.5657C13.9533 12.2533 13.9533 11.7467 14.2657 11.4343Z" fill="black"/>
<path d="M8.2657 11.4343L12.45 7.25C12.8642 6.83579 12.8642 6.16421 12.45 5.75C12.0358 5.33579 11.3642 5.33579 10.95 5.75L5.40712 11.2929C5.01659 11.6834 5.01659 12.3166 5.40712 12.7071L10.95 18.25C11.3642 18.6642 12.0358 18.6642 12.45 18.25C12.8642 17.8358 12.8642 17.1642 12.45 16.75L8.2657 12.5657C7.95328 12.2533 7.95328 11.7467 8.2657 11.4343Z" fill="black"/>
</svg>
																</span>
                                                    <!--end::Svg Icon-->
                                                </div>
                                            </div>
                                            <!--end::Timeline icon-->
                                            <!--begin::Timeline content-->
                                            <div class="timeline-content mb-10 mt-n2">
                                                <!--begin::Timeline heading-->
                                                <div class="overflow-auto pe-3">
                                                    <!--begin::Title-->
                                                    <div class="fs-5 fw-bold mb-2">
                                                        <span class="text-danger">
                                                          @if(\App\Models\abonne::where( 'id',$emprunt->abonne_id)->count()!=null)
                                                                {{
                                                                    \App\Models\abonne::find($emprunt->abonne_id)->nom}}
                                                                {{\App\Models\abonne::find($emprunt->abonne_id)->prenom}}
                                                            @endif

                                                        </span>

                                                        a rendu  le  livre
                                                        <span class="text-uppercase"> "                                                            @if(\App\Models\livre::where('id',$emprunt->livre_id)->count()!=null)
                                                                {{ \App\Models\livre::find($emprunt->livre_id)->titre }}
                                                            @endif"</span>
                                                        </div>                                                    <!--end::Title-->
                                                    <!--begin::Description-->
                                                    <div class="d-flex align-items-center mt-1 fs-6">
                                                        <!--begin::Info-->
                                                        <div class="text-muted me-2 fs-7">Emprunts at {{$emprunt->date}} by <span class="text-danger">  {{$emprunt->abonne_id }}  </span> </div>
                                                        <!--end::Info-->
                                                        <!--begin::User-->

                                                        <!--end::User-->
                                                    </div>
                                                    <!--end::Description-->
                                                </div>
                                                <!--end::Timeline heading-->
                                            </div>
                                            <!--end::Timeline content-->
                                        </div></td>
                                    @endif  </tr>
                                @endforeach
                            </tbody>
                        </table>
                    </div>
                    <!--end::Timeline-->
                </div>


                <!--begin::Tab panel-->
                <div id="kt_activity_month" class="card-body p-0 tab-pane fade" role="tabpanel" aria-labelledby="kt_activity_month_tab">
                    <div class="d-flex flex-row p-2">

                        <input type="text" data-kt-docs-table-filter="search_month" class="form-control form-control-solid w-250px ps-15 " placeholder="Search Emprunts"/>
                    </div>
                    <div class="timeline" >

                        <table id="kt_datatable_months" class="table align-middle table-row-dashed fs-6 gy-5">
                            <thead>
                            <tr class="fw-bold fs-6 text-muted">
                                <th>action</th>
                            </tr>
                            </thead>
                            <tbody class="text-gray-600 fw-bold">

                            @foreach($months as $emprunt)
                                <tr> @if($emprunt->status=='active')
                                        <td>    <div class="timeline-item content">

                                                <div class="timeline-icon symbol symbol-circle symbol-40px me-4">
                                                    <div class="symbol-label bg-light">
                                                        <!--begin::Svg Icon | path: icons/duotune/communication/com003.svg-->

                                                        <span class="svg-icon svg-icon-1 svg-icon-primary ">
												<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
										 <path opacity="0.5" d="M12.8956 13.4982L10.7949 11.2651C10.2697 10.7068 9.38251 10.7068 8.85731 11.2651C8.37559 11.7772 8.37559 12.5757 8.85731 13.0878L12.7499 17.2257C13.1448 17.6455 13.8118 17.6455 14.2066 17.2257L21.1427 9.85252C21.6244 9.34044 21.6244 8.54191 21.1427 8.02984C20.6175 7.47154 19.7303 7.47154 19.2051 8.02984L14.061 13.4982C13.7451 13.834 13.2115 13.834 12.8956 13.4982Z" fill="black"/>
                                        <path d="M7.89557 13.4982L5.79487 11.2651C5.26967 10.7068 4.38251 10.7068 3.85731 11.2651C3.37559 11.7772 3.37559 12.5757 3.85731 13.0878L7.74989 17.2257C8.14476 17.6455 8.81176 17.6455 9.20663 17.2257L16.1427 9.85252C16.6244 9.34044 16.6244 8.54191 16.1427 8.02984C15.6175 7.47154 14.7303 7.47154 14.2051 8.02984L9.06096 13.4982C8.74506 13.834 8.21146 13.834 7.89557 13.4982Z" fill="black"/>
                                       </svg>
											</span>
                                                        <!--end::Svg Icon-->
                                                    </div>
                                                </div>
                                                <!--end::Timeline icon-->
                                                <!--begin::Timeline content-->
                                                <div class="timeline-content mb-10 mt-n1">
                                                    <!--begin::Timeline heading-->
                                                    <div class="pe-3 mb-5">
                                                        <!--begin::Title-->
                                                        <div class="fs-5 fw-bold mb-2">
                                                        <span class="text-success">
                                                          @if(\App\Models\abonne::where( 'id',$emprunt->abonne_id)->count()!=null)
                                                                {{
                                                                    \App\Models\abonne::find($emprunt->abonne_id)->nom}}
                                                                {{\App\Models\abonne::find($emprunt->abonne_id)->prenom}}
                                                            @endif
                                                        </span>


                                                            a emprenté le  livre
                                                            <span class="text-uppercase"> "                                                            @if(\App\Models\livre::where('id',$emprunt->livre_id)->count()!=null)
                                                                    {{ \App\Models\livre::find($emprunt->livre_id)->titre }}
                                                                @endif"</span>
                                                        </div>
                                                        <!--end::Title-->
                                                        <!--begin::Description-->
                                                        <div class="d-flex align-items-center mt-1 fs-6">
                                                            <!--begin::Info-->
                                                            <div class="text-muted me-2 fs-7">Emprunts at {{$emprunt->date}} by <span class="text-success">  {{$emprunt->abonne_id }}  </span> </div>
                                                            <!--end::Info-->
                                                            <!--begin::User-->

                                                            <!--end::User-->
                                                        </div>
                                                        <!--end::Description-->
                                                    </div>
                                                    <!--end::Timeline heading-->
                                                    <!--begin::Timeline details-->
                                                    <!--end::Timeline details-->
                                                </div>
                                                <!--end::Timeline content-->
                                            </div></td>
                                    @else
                                        <td>    <div class="timeline-item content">
                                                <!--begin::Timeline line-->
                                                <!--end::Timeline line-->
                                                <!--begin::Timeline icon-->
                                                <div class="timeline-icon symbol symbol-circle symbol-40px">
                                                    <div class="symbol-label bg-light">
                                                        <!--begin::Svg Icon | path: icons/duotune/communication/com009.svg-->
                                                        <span class="svg-icon svg-icon-1 svg-icon-danger ">
	<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
<path opacity="0.5" d="M14.2657 11.4343L18.45 7.25C18.8642 6.83579 18.8642 6.16421 18.45 5.75C18.0358 5.33579 17.3642 5.33579 16.95 5.75L11.4071 11.2929C11.0166 11.6834 11.0166 12.3166 11.4071 12.7071L16.95 18.25C17.3642 18.6642 18.0358 18.6642 18.45 18.25C18.8642 17.8358 18.8642 17.1642 18.45 16.75L14.2657 12.5657C13.9533 12.2533 13.9533 11.7467 14.2657 11.4343Z" fill="black"/>
<path d="M8.2657 11.4343L12.45 7.25C12.8642 6.83579 12.8642 6.16421 12.45 5.75C12.0358 5.33579 11.3642 5.33579 10.95 5.75L5.40712 11.2929C5.01659 11.6834 5.01659 12.3166 5.40712 12.7071L10.95 18.25C11.3642 18.6642 12.0358 18.6642 12.45 18.25C12.8642 17.8358 12.8642 17.1642 12.45 16.75L8.2657 12.5657C7.95328 12.2533 7.95328 11.7467 8.2657 11.4343Z" fill="black"/>
</svg>
																</span>
                                                        <!--end::Svg Icon-->
                                                    </div>
                                                </div>
                                                <!--end::Timeline icon-->
                                                <!--begin::Timeline content-->
                                                <div class="timeline-content mb-10 mt-n2">
                                                    <!--begin::Timeline heading-->
                                                    <div class="overflow-auto pe-3">
                                                        <!--begin::Title-->
                                                        <div class="fs-5 fw-bold mb-2">
                                                        <span class="text-danger">
                                                          @if(\App\Models\abonne::where( 'id',$emprunt->abonne_id)->count()!=null)
                                                                {{
                                                                    \App\Models\abonne::find($emprunt->abonne_id)->nom}}
                                                                {{\App\Models\abonne::find($emprunt->abonne_id)->prenom}}
                                                            @endif
                                                        </span>

                                                            a rendu  le  livre
                                                            <span class="text-uppercase"> "                                                            @if(\App\Models\livre::where('id',$emprunt->livre_id)->count()!=null)
                                                                    {{ \App\Models\livre::find($emprunt->livre_id)->titre }}
                                                                @endif"</span>
                                                        </div>                                                    <!--end::Title-->
                                                        <!--begin::Description-->
                                                        <div class="d-flex align-items-center mt-1 fs-6">
                                                            <!--begin::Info-->
                                                            <div class="text-muted me-2 fs-7">Emprunts at {{$emprunt->date}} by <span class="text-danger">  {{$emprunt->abonne_id }}  </span> </div>
                                                            <!--end::Info-->
                                                            <!--begin::User-->

                                                            <!--end::User-->
                                                        </div>
                                                        <!--end::Description-->
                                                    </div>
                                                    <!--end::Timeline heading-->
                                                </div>
                                                <!--end::Timeline content-->
                                            </div></td>
                                    @endif  </tr>
                            @endforeach
                            </tbody>
                        </table>


                    </div>
                </div>
                <!--end::Tab panel-->
                <!--begin::Tab panel-->
                <div id="kt_activity_year" class="card-body p-0 tab-pane fade active show" role="tabpanel" aria-labelledby="kt_activity_year_tab">
                    <div class="d-flex flex-row p-2">

                        <input type="text" data-kt-docs-table-filter="search_years" class="form-control form-control-solid w-250px ps-15 " placeholder="Search Emprunts"/>
                    </div>
                    <div class="timeline" >

                        <table id="kt_datatable_years" class="table align-middle table-row-dashed fs-6 gy-5">
                            <thead>
                            <tr class="fw-bold fs-6 text-muted">
                                <th>action</th>
                            </tr>
                            </thead>
                            <tbody class="text-gray-600 fw-bold">

                            @foreach($years as $emprunt)
                                <tr> @if($emprunt->status=='active')
                                        <td>    <div class="timeline-item content">

                                                <div class="timeline-icon symbol symbol-circle symbol-40px me-4">
                                                    <div class="symbol-label bg-light">
                                                        <!--begin::Svg Icon | path: icons/duotune/communication/com003.svg-->

                                                        <span class="svg-icon svg-icon-1 svg-icon-primary ">
												<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
										 <path opacity="0.5" d="M12.8956 13.4982L10.7949 11.2651C10.2697 10.7068 9.38251 10.7068 8.85731 11.2651C8.37559 11.7772 8.37559 12.5757 8.85731 13.0878L12.7499 17.2257C13.1448 17.6455 13.8118 17.6455 14.2066 17.2257L21.1427 9.85252C21.6244 9.34044 21.6244 8.54191 21.1427 8.02984C20.6175 7.47154 19.7303 7.47154 19.2051 8.02984L14.061 13.4982C13.7451 13.834 13.2115 13.834 12.8956 13.4982Z" fill="black"/>
                                        <path d="M7.89557 13.4982L5.79487 11.2651C5.26967 10.7068 4.38251 10.7068 3.85731 11.2651C3.37559 11.7772 3.37559 12.5757 3.85731 13.0878L7.74989 17.2257C8.14476 17.6455 8.81176 17.6455 9.20663 17.2257L16.1427 9.85252C16.6244 9.34044 16.6244 8.54191 16.1427 8.02984C15.6175 7.47154 14.7303 7.47154 14.2051 8.02984L9.06096 13.4982C8.74506 13.834 8.21146 13.834 7.89557 13.4982Z" fill="black"/>
                                       </svg>
											</span>
                                                        <!--end::Svg Icon-->
                                                    </div>
                                                </div>
                                                <!--end::Timeline icon-->
                                                <!--begin::Timeline content-->
                                                <div class="timeline-content mb-10 mt-n1">
                                                    <!--begin::Timeline heading-->
                                                    <div class="pe-3 mb-5">
                                                        <!--begin::Title-->
                                                        <div class="fs-5 fw-bold mb-2">
                                                        <span class="text-success">
                                                          @if(\App\Models\abonne::where( 'id',$emprunt->abonne_id)->count()!=null)
                                                                {{
                                                                    \App\Models\abonne::find($emprunt->abonne_id)->nom}}
                                                                {{\App\Models\abonne::find($emprunt->abonne_id)->prenom}}
                                                            @endif
                                                        </span>
                                                            a emprenté le  livre
                                                            <span class="text-uppercase"> "                                                            @if(\App\Models\livre::where('id',$emprunt->livre_id)->count()!=null)
                                                                    {{ \App\Models\livre::find($emprunt->livre_id)->titre }}
                                                                @endif"</span>
                                                        </div>
                                                        <!--end::Title-->
                                                        <!--begin::Description-->
                                                        <div class="d-flex align-items-center mt-1 fs-6">
                                                            <!--begin::Info-->
                                                            <div class="text-muted me-2 fs-7">Emprunts at {{$emprunt->date}} by <span class="text-success">  {{$emprunt->abonne_id }}  </span> </div>
                                                            <!--end::Info-->
                                                            <!--begin::User-->

                                                            <!--end::User-->
                                                        </div>
                                                        <!--end::Description-->
                                                    </div>
                                                    <!--end::Timeline heading-->
                                                    <!--begin::Timeline details-->
                                                    <!--end::Timeline details-->
                                                </div>
                                                <!--end::Timeline content-->
                                            </div></td>
                                    @else
                                        <td>    <div class="timeline-item content">
                                                <!--begin::Timeline line-->
                                                <!--end::Timeline line-->
                                                <!--begin::Timeline icon-->
                                                <div class="timeline-icon symbol symbol-circle symbol-40px">
                                                    <div class="symbol-label bg-light">
                                                        <!--begin::Svg Icon | path: icons/duotune/communication/com009.svg-->
                                                        <span class="svg-icon svg-icon-1 svg-icon-danger ">
	<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
<path opacity="0.5" d="M14.2657 11.4343L18.45 7.25C18.8642 6.83579 18.8642 6.16421 18.45 5.75C18.0358 5.33579 17.3642 5.33579 16.95 5.75L11.4071 11.2929C11.0166 11.6834 11.0166 12.3166 11.4071 12.7071L16.95 18.25C17.3642 18.6642 18.0358 18.6642 18.45 18.25C18.8642 17.8358 18.8642 17.1642 18.45 16.75L14.2657 12.5657C13.9533 12.2533 13.9533 11.7467 14.2657 11.4343Z" fill="black"/>
<path d="M8.2657 11.4343L12.45 7.25C12.8642 6.83579 12.8642 6.16421 12.45 5.75C12.0358 5.33579 11.3642 5.33579 10.95 5.75L5.40712 11.2929C5.01659 11.6834 5.01659 12.3166 5.40712 12.7071L10.95 18.25C11.3642 18.6642 12.0358 18.6642 12.45 18.25C12.8642 17.8358 12.8642 17.1642 12.45 16.75L8.2657 12.5657C7.95328 12.2533 7.95328 11.7467 8.2657 11.4343Z" fill="black"/>
</svg>
																</span>
                                                        <!--end::Svg Icon-->
                                                    </div>
                                                </div>
                                                <!--end::Timeline icon-->
                                                <!--begin::Timeline content-->
                                                <div class="timeline-content mb-10 mt-n2">
                                                    <!--begin::Timeline heading-->
                                                    <div class="overflow-auto pe-3">
                                                        <!--begin::Title-->
                                                        <div class="fs-5 fw-bold mb-2">
                                                        <span class="text-danger">
                                                          @if(\App\Models\abonne::where( 'id',$emprunt->abonne_id)->count()!=null)
                                                                {{
                                                                    \App\Models\abonne::find($emprunt->abonne_id)->nom}}
                                                                {{\App\Models\abonne::find($emprunt->abonne_id)->prenom}}
                                                            @endif
                                                        </span>

                                                            a rendu  le  livre
                                                            <span class="text-uppercase"> "                                                            @if(\App\Models\livre::where('id',$emprunt->livre_id)->count()!=null)
                                                                    {{ \App\Models\livre::find($emprunt->livre_id)->titre }}
                                                                @endif"</span>
                                                        </div>                                                    <!--end::Title-->
                                                        <!--begin::Description-->
                                                        <div class="d-flex align-items-center mt-1 fs-6">
                                                            <!--begin::Info-->
                                                            <div class="text-muted me-2 fs-7">Emprunts at {{$emprunt->date}} by <span class="text-danger">  {{$emprunt->abonne_id }}  </span> </div>
                                                            <!--end::Info-->
                                                            <!--begin::User-->

                                                            <!--end::User-->
                                                        </div>
                                                        <!--end::Description-->
                                                    </div>
                                                    <!--end::Timeline heading-->
                                                </div>
                                                <!--end::Timeline content-->
                                            </div></td>
                                    @endif  </tr>
                            @endforeach
                            </tbody>
                        </table>

                    </div>
                </div>
                <!--end::Tab panel-->
            </div>
            <!--end::Tab Content-->
        </div>
        <!--end::Card body-->
    </div>
    <script src="assets/plugins/custom/datatables/datatables.bundle.js"></script>

    <script>


       days= $("#kt_datatable_days").DataTable({
            searching: true,
           ordering: false ,

        });

       const filterSearch = document.querySelector('[data-kt-docs-table-filter="search"]');
       filterSearch.addEventListener('keyup', function (e) {
           days.search(e.target.value).draw();
       });

       months=$("#kt_datatable_months").DataTable({
            searching: true,
           ordering: false ,

        });
       const filterSearchmonths = document.querySelector('[data-kt-docs-table-filter="search_month"]');
       filterSearchmonths.addEventListener('keyup', function (e) {
           months.search(e.target.value).draw();
       });
       years= $("#kt_datatable_years").DataTable({
            ordering: false ,
            searching: true,
       });
       const filtersearch_years = document.querySelector('[data-kt-docs-table-filter="search_years"]');
       filtersearch_years.addEventListener('keyup', function (e) {
           years.search(e.target.value).draw();
       });
    </script>
</div>
