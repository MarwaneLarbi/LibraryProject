...
(function(window){
    ...
    function lignePaginate(){
        ...
        var pageButtons = function(numberOfPage,currentPage) {
             ...
             **/
            let buttons = "<input type='button' value='← prev' class='page-item previous' onclick='paginate.sort("+(currentPage - 1)+")' "+prevDisabled+">";
            let buttonNumberOfPage = "<input type='button btn-primary' value='" + currentPage + ' - ' + numberOfPage + "' disabled>";

            for (let $i=1; $i<=numberOfPage;$i++){
                if(numberOfPage > 10){
                    buttons += paginationMoreThatTenPage($i,numberOfPage);
                }else{
                    buttons += "<input type='button' class='btn-primary' id='id"+$i+"'value='"+$i+"' onclick='paginate.sort("+$i+")'>";
                }
            }
        ...
        var paginationMoreThatTenPage = function(iterator,numberOfPage){
            ...
            if(iterator > referenceForTheLast || iterator < 5){
                return "<li class='page-item' type='button' id='id"+iterator+"'value='"+iterator+"' onclick='paginate.sort("+iterator+")'> </li>";
                ...
                return "<li  id='id"+iterator+"'value='"+iterator+"' style='display: none' onclick='paginate.sort("+iterator+")'></li>";
            }
        }
        ...
        var launchPaginate = function(){
            ...
            if (pageCount > 1) {
                ...
                table.insertAdjacentHTML("afterend","<ul  id='buttons' class='pagination'></ul>");
